...
if __name__ == "__main__":
    ...
    print("average time:", total_times/14)
    print("\nSaving images:")
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        if detections is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                ...
                print("\t+ Label: %s, Conf: %.5f, At: %.3f %.3f %.3f %.3f" % (classes[int(cls_pred)], cls_conf.item(), x1,y1,x2,y2))
                box_w = x2 - x1
                box_h = y2 - y1
        ...
        plt.savefig(f"data/custom/output/{filename}.png", bbox_inches="tight", pad_inches=0.0)
        plt.close()
